		...
		try {
			...
			for (FileItem item : files) {
				...
				if (item.getName().indexOf(":\\") >= 0) {
					uploadFileName=uploadFileName.substring(uploadFileName.lastIndexOf("\\"),uploadFileName.length());
				}
				else{
				...
				/*} else if (fileExt.equalsIgnoreCase("zip")) {
					...
					String bundlePath= tmpRoot + uploadFileName.substring(0,uploadFileName.lastIndexOf("."))+"\\";
					ZipReader zipReader = new ZipReader(filePath, tmpRoot);
					zipReader.read();
					ProjectCompiler projectCompiler=new ProjectCompiler(bundlePath,"test");
