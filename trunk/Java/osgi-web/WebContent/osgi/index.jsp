...
<%
	...
	// 			"Compiler", "CompressUtil", "CXFLib", "IOUtil",
	// 			"javax.servlet", "javax.servlet.jsp", "json_plugin",
	// 			"org.apache.commons.el", "org.apache.commons.logging",
	// 			"org.apache.jasper", "org.apache.log4j",
	// 			"org.eclipse.osgi", "org.eclipse.osgi.services",
	// 			"org.springframework.aop", "org.springframework.beans",
	...
	// 			"org.springframework.web.servlet", "OSGIWebAdmin",
	// 			"cxf-dosgi-ri-singlebundle-distribution",
	// 			"WebServiceFactory");
	List<String> systemBundles = new ArrayList<>();
%>
...
<body>
	<div class="head">
		<span>基于OSGI的动态组件管理平台</span>&nbsp;&nbsp;&nbsp;<span><a
			href="webservice/index" target="_blank"
			style="font-size: 15px; color: #FFF;">Web服务管理页</a></span>
	</div>
	...
	<div id="uploaddiv">
		...
		<div style="width: auto; float: right;">
			<input type="button" id="uploadbutton" value="安装新的插件"
				onmouseover="$(this).css('background-color','#900');"
				onmouseout="$(this).css('background-color','#222');"
				onclick="showFileUpload(this);" />
		</div>
	</div>
	<div id="bundlediv">
		<table class="table" id="bundles">
			...
			<tbody>
				...
				<tr>
					...
					<td><span class="bundlename"><%=bundle.getSymbolicName()%></span>&nbsp;&nbsp;&nbsp;<img
						width="14px" src="image/find.png"
						onclick="loadBundleInfo(this,<%=bundleId%>);" /></td>
 ...
 %><a href="javascript:void(0);" onclick="stopBundle(<%=bundleId%>);">Stop</a>|<%
 	} else if (iState == Bundle.RESOLVED
 					|| iState == Bundle.INSTALLED) {
 %><a href="javascript:void(0);" onclick="startBundle(<%=bundleId%>);">Start</a>|<%
 	}
 %><a href="javascript:void(0);"
						onclick="showUpdateBundle(this,<%=bundleId%>);">Update</a> <%
 	if (!systemBundles.contains(bundle.getSymbolicName())) {
 %>|<a href="javascript:void(0);"
						onclick="uninstallBundle(<%=bundleId%>);">Uninstall</a> <%
 	}
 		}
 %>
