...
public class UpdateBundle extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (FileItem item : files) {
				...
				// update bundle from specified input stream
				BundleFactory.update(id, new FileInputStream(file));
				break;
				/*
				 * } else if (fileExt.equalsIgnoreCase("zip")) { // Install Web
				 * Bundle here String tmpRoot=bundleRoot + "temp\\"; String
				 * bundlePath= tmpRoot +
				 * uploadFileName.substring(0,uploadFileName
				 * .lastIndexOf("."))+"\\"; ZipReader zipReader = new
				 * ZipReader(filePath, tmpRoot); zipReader.read();
				 * ProjectCompiler projectCompiler=new
				 * ProjectCompiler(bundlePath,"test");
		 ...
		 * response.getWriter(); try { BundleFactory.update(id);
		 * out.print(true); } catch (BundleException e) { out.print(false);
	 ...
	 * File("e:\\bundles\\"); try { FileInputStream input=new
	 * FileInputStream(file); InputStreamReader isr=new
	 * InputStreamReader(input); BufferedReader br=new BufferedReader(isr);
	 * String tmp; String str=""; while((tmp=br.readLine())!=null){ str+=tmp; }
